    ...
    if(type == "bst")
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            if(intent == "inorder")
            {
				...
				duration<double> timePass = duration_cast<duration<double>>(tEnd - tStart);
				Timing[6] += timePass.count()*1000;
            }
        }

		
		cerr<<"inserts: "<<inserts<<" avgTime: "<<Timing[0]<<"ms\n";
		cerr<<"deletes: "<<deletes<<" avgTime: "<<Timing[1]<<"ms\n";
		cerr<<"finds: "<<finds<<" avgTime: "<<Timing[2]<<"ms\n";
		cerr<<"mins: "<<mins<<" avgTime: "<<Timing[3]<<"ms\n";
		cerr<<"maxs: "<<maxs<<" avgTime: "<<Timing[4]<<"ms\n";
		cerr<<"successors: "<<successors<<" avgTime: "<<Timing[5]<<"ms\n";
		cerr<<"inorders: "<<inorders<<" avgTime: "<<Timing[6]<<"ms\n";
		cerr<<"compares: "<<compares<<"\n";
		cerr<<"Max #elements: "<<maxElements<<"\n";
		cerr<<"cur #elements: "<<curElements<<"\n";
    }
    // Red-Black Tree
    else if(type == "rbt")
    {
